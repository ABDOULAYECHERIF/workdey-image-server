...
const STYLE = `Premium professional photograph, portrait orientation, suitable as a Facebook advertisement image. 
Mostly a beautiful real photo with deep palm green (#0a8508) and vibrant orange (#f7a814) brand accents. 
Minimal text overlay only — less than 20% of the image area is text (Facebook ad policy). 
No paragraphs, no fake body text, no gibberish. Clean, photographic, professional. Award-winning quality.`;
...
function buildFlyerPrompt(postType, d, seed, flyer) {
  ...
  // Minimal text: one small headline + small logo. Mostly visual (Facebook ad-friendly: <20% text area).
  return `A premium, photographic recruitment campaign image featuring ${scene}. 
The image is mostly visual — a beautiful professional photo. 
Small clean text overlay with just the short headline "${headline}" in one corner, and a small "WorkDey" wordmark. 
Keep text minimal, occupying less than 20% of the image (Facebook advertising standard). Mostly imagery, little text. 
${STYLE}`;
}
